...
class response_training_model(object):
    ...
    def estimated_performance_class(self, dataset, target, model, validation, list_metric):
        ...
        cf_matrix = []
        for row in confusion_matrix_response:
            list_row = []
            for index_a, a in enumerate(row):
                list_row.append(int(a))
            cf_matrix.append(list_row)
        response = {"performance": performance, "confusion_matrix": cf_matrix}
        return response
    ...
    def estimated_metric_regresion_models(self, dataset, target, model, validation, list_metric):
        ...
        response = {"performances": {"training": performance, "kendall": kendall_value, "pearson": pearson_value,
                                     "spearman": spearma_value}, "predict_label": list(predict_values), "real_values": list(target)}
        return response
