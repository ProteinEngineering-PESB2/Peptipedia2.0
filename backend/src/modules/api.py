...
class api:
    ...
    def api_alignment():
        ...
        align = alignment(
            ...
            is_file,
            is_json,
            int(config["blast"]["max_sequences"]),
    ...
    def api_msa():
        ...
        msa = multiple_sequence_alignment(
            ...
            is_file,
            is_json,
            int(config["msa"]["max_sequences"]),
            int(config["msa"]["min_sequences"]),
        )
        check = msa.check
    ...
    def api_phisicochemical():
        data, options, is_json, is_file = interface.parse_information_with_options(
            request
        )
        modlamp = modlamp_descriptor(
            data,
            options,
            temp_folder,
            is_file,
            is_json,
            int(config["physicochemical"]["max_sequences"]),
            int(config["physicochemical"]["min_sequences"]),
        )
        check = modlamp.check
        if check["status"] == "error":
            return check
    ...
    def api_gene_ontology():
        ...
        go = gene_ontology(
            ...
            is_file,
            is_json,
            int(config["gene_ontology"]["max_sequences"]),
            int(config["gene_ontology"]["min_sequences"]),
        )
        check = go.check
        if check["status"] == "error":
            return check
    ...
    def api_pfam():
        ...
        pf = pfam(
            ...
            is_file,
            is_json,
            int(config["pfam"]["max_sequences"]),
            int(config["pfam"]["min_sequences"]),
        )
        check = pf.check
        if check["status"] == "error":
            return check
    ...
    def api_frequency():
        data, is_json, is_file = interface.parse_information_no_options(request)
        frequency_object = frequency_analysis(
            data,
            temp_folder,
            is_file,
            is_json,
            int(config["frequency"]["max_sequences"]),
            int(config["frequency"]["min_sequences"]),
        )
    ...
    def api_encoding():
        ...
        code = encoding(
            ...
            is_file,
            is_json,
            int(config["encoding"]["max_sequences"]),
            int(config["encoding"]["min_sequences"]),
            path_aa_index,
        )
        check = code.check
        if check["status"] == "error":
    ...
    def api_clustering():
        ...
        clustering_object = unsupervised_algorithms(
            data,
            options,
            static_folder,
            temp_folder,
            is_file,
            is_json,
            int(config["clustering"]["max_sequences"]),
            int(config["clustering"]["min_sequences"]),
            path_aa_index,
        )
    ...
    def api_supervised_learning():
        ...
        sl = supervised_algorithms(
            ...
            is_file,
            is_json,
            int(config["supervised_learning"]["max_sequences"]),
            int(config["supervised_learning"]["min_sequences"]),
            path_aa_index,
        )
        check = sl.check
        if check["status"] == "error":
    ...
    def api_use_model():
        ...
        use = use_model(
            ...
            is_file,
            is_json,
            int(config["use_model"]["max_sequences"]),
            int(config["use_model"]["min_sequences"]),
            path_aa_index,
        )
        check = use.check
        if check["status"] == "error":
