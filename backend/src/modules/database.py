...
class database:
    ...
    def select_peptides(self, query, limit, offset):
        ...
        limited_query = "{} order by idpeptide limit {} offset {}".format(query, limit, offset)
        try:
    ...
    def get_all_gene_ontology(self, sub_string, limit):
        ...
        if sub_string == None:
            query = """select id_go, term, source
                        from gene_ontology
                        limit {}""".format(self.max_items)
        else:
            query = """select id_go, term, source 
                        from gene_ontology 
                        where UPPER(term) like UPPER('%{}%') 
                        limit {}""".format(sub_string, self.max_items)
    ...
    def get_all_pfam(self, sub_string, limit):
        ...
        if sub_string == None:
            query = """select id_pfam, name, type
                        from pfam limit {}""".format(self.max_items)
        else:
    ...
    def get_all_taxonomy(self, sub_string, limit):
        ...
        if sub_string == None:
            query = """select idtaxonomy, name, tax_type 
                        from taxonomy limit {}""".format(self.max_items)
        else:
            query = """select idtaxonomy, name, tax_type 
                        from taxonomy 
                        where UPPER(name) like UPPER('%{}%') 
    ...
    def get_go_from_peptide(self, idpeptide):
        ...
        data = pd.read_sql("""select accession, term, description, source, probability 
                                from peptide_has_go phg 
                                join gene_ontology go 
                                on go.id_go = phg.id_go 
                                and idpeptide = {}""".format(idpeptide), con=self.conn)
        return {
            "status": "success",
            "data": data.values.tolist(),
            "columns": data.columns.tolist()
    ...
    def get_pfam_from_peptide(self, idpeptide):
        ...
        data = pd.read_sql("""select accession, name, type 
    ...
    def get_tax_from_peptide(self, idpeptide):
        ...
        data = pd.read_sql("""select name, tax_type 
    ...
    def get_info_from_peptide(self, idpeptide):
        ...
        data = pd.read_sql("""select sequence, length, molecular_weight, 
                                charge_density, isoelectric_point, 
                                charge from peptide 
                                where idpeptide = {}""".format(idpeptide), con=self.conn)
        return json.loads(data.to_json(orient="records")) 

    def get_sequence_from_peptide(self, idpeptide):
        #Gets the sequence of a peptide (str)
        data = pd.read_sql("""select sequence 
                                from peptide 
                                where idpeptide = {}""".format(idpeptide), con=self.conn)
    ...
    def get_act_from_peptide(self, idpeptide):
        ...
        data = pd.read_sql("""select act.name, pha.is_predicted 
                                from peptide_has_activity pha 
                                join activity act on pha.idactivity = act.idactivity 
                                and pha.idpeptide = {}""".format(idpeptide), con=self.conn)
        return {
            "status": "success", 
            "data": data.values.tolist(), 
            "columns": data.columns.tolist()
    ...
    def get_patent_from_peptide(self, idpeptide):
        ...
        data = pd.read_sql("""select patent 
                                from patent 
                                where patent.idpeptide = {}""".format(idpeptide), con=self.conn)
        return {
            "status": "success", 
            "data": data.values.tolist(), 
            "columns": data.columns.tolist()
    ...
    def get_db_from_peptide(self, idpeptide):
        ...
        data = pd.read_sql("""select db.name as db, i.name as id 
    ...
    def get_uniprot(self, idpeptide):
        ...
        data = pd.read_sql("""select structure as uniprot 
                                from peptide 
                                where idpeptide = {};""".format(idpeptide), con=self.conn)
        try:
    ...
    def get_db_statistics(self):
        ...
        data = pd.read_sql("""select db.name as database, 
                                COUNT(phdhi.idpeptide) as peptides, db.app_url 
                                from peptide_has_db_has_index phdhi 
                                join db on db.id_db = phdhi.id_db
                                group by db.name, db.app_url
                                order by peptides desc""", con=self.conn)
        return {
            "status": "success", 
            "data": data.values.tolist(), 
            "columns": data.columns.tolist()
    ...
    def get_all_act_statistics(self):
        ...
        data = pd.read_sql("""select act.idactivity , act.name as Activity, COUNT(pha.idpeptide) as Peptides 
                                from peptide_has_activity pha
    ...
    def get_specific_act_statistics(self, idactivity):
        ...
        data = pd.read_sql("""select p.length, p.charge, p.molecular_weight, 
                                ...
                                and pha.idactivity = {}""".format(idactivity), con=self.conn)
        description = data.describe()
        length = {
            a: round(b, 3) for a, b in zip(description.index.tolist(), 
                                            description.length.values.tolist())
        }
        charge = {
            a: round(b, 3) for a, b in zip(description.charge.index.tolist(), 
                                            description.charge.values.tolist())
        }
        molecular_weight = {
            a: round(b, 3) for a, b in zip(description.molecular_weight.index.tolist(),
                                            description.molecular_weight.values.tolist())
        }
        charge_density = {
            a: round(b, 5) for a, b in zip(description.charge_density.index.tolist(),
                                            description.charge_density.values.tolist())
        }
        isoelectric_point = {
            a: round(b, 4) for a, b in zip(description.isoelectric_point.index.tolist(), 
    ...
    def get_tree(self):
        ...
        resume = pd.read_sql("""select act.name as Activity, COUNT(pha.idpeptide) as Peptides 
                    from peptide_has_activity pha
                    join activity act on act.idactivity = pha.idactivity 
    ...
    def get_general_act_statistic(self):
        ...
        all = pd.read_sql("""select COUNT(idpeptide) from peptide p""",
                            con=self.conn).values[0][0]
        predicted = pd.read_sql("""select COUNT(distinct idpeptide)
                                    from peptide_has_activity pha
                                    where pha.is_predicted = true""",
                                    con=self.conn).values[0][0]
        labeled = pd.read_sql("""select COUNT(distinct idpeptide)
                                    from peptide_has_activity pha
