...
class alignment(config_tool):
    ...
    def get_alignments(self):
        ...
        for id, j in zip(self.ids, splitted[1:]):
            ...
            inicio = j.find("Query")
            row = j[inicio:]
            query = "".join([split("\s+", q)[2] for q in row.split("\n") if "Query" in q])
            sbjct = "".join([split("\s+", sb)[2] for sb in row.split("\n") if "Sbjct" in sb])
            response[id] = [{"id": 1, "label": id, "sequence": sbjct}, {"id": 2, "label": id_query, "sequence": query}]
        return response